...
def _get_features(
    ...
    if with_label:
        punct_labels = [pad_id]
        punct_query_labels = [punct_label_ids[lab] for lab in punct_labels_line]

        capit_labels = [pad_id]
        capit_query_labels = [capit_label_ids[lab] for lab in capit_labels_line]
...
def get_features(
    ...
    if not with_label:
        capit_labels_lines = [""] * len(queries)
        punct_labels_lines = [""] * len(queries)
        pad_id = 0
    with Pool(processes=24) as p:
        get_features_output = p.starmap(
            functools.partial(
                _get_features,
                tokenizer=tokenizer,
            ...
            tqdm(
                zip(queries, punct_labels_lines, capit_labels_lines), total=len(queries)
            ),
        )
    for i in get_features_output:
        sent_lengths.append(i[0])
        all_subtokens.append(i[1])
        all_loss_mask.append(i[2])
        all_subtokens_mask.append(i[3])
        all_input_mask.append(i[4])
        if with_label:
            punct_all_labels.append(i[5])
            capit_all_labels.append(i[6])
    ...
    for i, subtokens in enumerate(all_subtokens):
        if len(subtokens) > max_seq_length:
            ...
            if with_label:
                punct_all_labels[i] = [pad_id] + punct_all_labels[i][
                    -max_seq_length + 1 :
                ]
                capit_all_labels[i] = [pad_id] + capit_all_labels[i][
                    -max_seq_length + 1 :
                ]
            too_long_count += 1
    ...
    def __init__(
        ...
        if get_label_frequencies:
            self.punct_label_frequencies = self._calculate_label_frequencies(
                self.punct_all_labels, data_dir, "punct"
            )
            self.capit_label_frequencies = self._calculate_label_frequencies(
                self.capit_all_labels, data_dir, "capit"
            )

    def _calculate_label_frequencies(
        self, all_labels: List[int], data_dir: str, name: str
    ) -> Dict[str, float]:
        ...
        _, label_frequencies, _ = get_label_stats(
            merged_labels, data_dir + "/label_count_" + name + ".tsv"
        )
